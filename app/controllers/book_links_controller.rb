class BookLinksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @link.save
        ...
        format.turbo_stream { render turbo_stream: turbo_stream.append('cta-links', partial: "components/cta_link", locals: {cta_link: @link}) }
  ...
  def update
      ...
      if @book.update(link_params)
        @link.tag_list = params[:tag_list] unless params[:taglist].nil? || params[:tag_list].blank?
        format.turbo_stream { render turbo_stream: turbo_stream.append(dom_id(@link), partial: "components/cta_link", locals: {cta_link: @link}) }
