...
class WritingGoalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create bucket' do
    assert_difference('WritingGoal.count') do
      post buckets_url,
           params: { bucket: { book_id: @bucket.book_id, finish_on: @bucket.finish_on, start_on: @bucket.start_on,
                               summary: @bucket.summary, target: @bucket.target } }
    end
